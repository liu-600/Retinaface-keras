...
#   种类预测（是否包含人脸）
#---------------------------------------------------#
def ClassHead(inputs, num_anchors=2):#num_anchors先验框数量，*2判断是否有人脸
    outputs = Conv2D(num_anchors*2, kernel_size=1, strides=1)(inputs)
    return Activation("softmax")(Reshape([-1,2])(outputs))#softmax判断人脸概率

#---------------------------------------------------#
#   预测框预测
#---------------------------------------------------#
def BboxHead(inputs, num_anchors=2): #先验框和它的4个调整参数
...
#   人脸关键点预测
#---------------------------------------------------#
def LandmarkHead(inputs, num_anchors=2):#5个人脸关键点的x,y轴调整参数
...
def RetinaFace(cfg, backbone="mobilenet"):
    ...
    if backbone == "mobilenet":
        C3, C4, C5 = MobileNet(inputs) # 64 128 256
    ...
    if (cfg['out_channel'] <= 64):
        leaky = 0.1
    #-------------------------------------------#
    #   获得三个shape的有效特征层（上采样，特征融合，两次卷积）
    #   分别是P3  80, 80, 64
    #         P4  40, 40, 64
    #         P5  20, 20, 64
    #-------------------------------------------# cfg['out_channel']--64  设置通道数  调整通道数
    ...
    #   P4  40, 40, 64
    #-------------------------------------------#
    P5_upsampled = UpsampleLike(name='P5_upsampled')([P5, P4]) #将p5长和宽调整到p4大小
    ...
    P3 = Conv2D_BN_Leaky(cfg['out_channel'], kernel_size=3, strides=1, padding='same', name='Conv_P3_merged', leaky=leaky)(P3) #3*3卷积进行特征整合
    
    SSH1 = SSH(P3, cfg['out_channel'], leaky=leaky)
    SSH2 = SSH(P4, cfg['out_channel'], leaky=leaky)
    SSH3 = SSH(P5, cfg['out_channel'], leaky=leaky)

    SSH_all = [SSH1,SSH2,SSH3] #循环
