...
class BBoxUtility(object):
    ...
    def decode_boxes(self, mbox_loc, mbox_ldm, mbox_anchorbox):#先验框调整参数，人脸关键点调整参数
        #-----------------------------------------------------#
        #   获得先验框的宽与高
        #-----------------------------------------------------#
        ...
        #-----------------------------------------------------#
        #   真实框距离先验框中心的xy轴偏移情况        真实框（调整后的框）--预测框
        #-----------------------------------------------------#
        decode_bbox_center_x = mbox_loc[:, 0] * anchor_width * 0.1 #偏移部分
...
#完成先验框准换成预测框的部分
        ...
        #-----------------------------------------------------#
        mbox_ldm            = mbox_ldm.reshape([-1, 5, 2])#-1每个先验框，5个人脸关键点，2关键点调整参数
    ...
    def detection_out(self, predictions, mbox_anchorbox, confidence_threshold=0.5):
        #---------------------------------------------------#
        #   mbox_loc是回归预测结果  ，对先验框调整获得预测框
        #---------------------------------------------------#
        mbox_loc    = predictions[0][0]
        #---------------------------------------------------#
        #   mbox_conf是人脸种类预测结果  ，判断是否有人脸
        #---------------------------------------------------#
        mbox_conf   = predictions[1][0][:, 1:2]
        #---------------------------------------------------#
        #   mbox_ldm是人脸关键点预测结果  ，调整先验框中心获得5个人脸关键点
        #---------------------------------------------------#
...
def Alignment_1(img,landmark):
    ...
    center = (img.shape[1]//2, img.shape[0]//2)#找旋转中心（大概在鼻尖）
